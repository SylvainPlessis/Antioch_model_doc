...
\geometry{paperwidth=15cm,paperheight=6.8cm,left=2pt,right=2pt,top=2pt,bottom=2pt}
\pagestyle{empty}
...
parameter/.style={font=\footnotesize, fill=white, inner sep=1pt, outer sep=0pt, opacity=0.8, text opacity=1},
...
\matrix[objects,right = 1cm 
             ,align=left]    (kinmod)   at (kinmol.east)        {\node{Hercourt Essen}; & \node{Berthelot};\\
                                                                 \node{Arrhenius};      & \node{Kooij};\\
                                                                 \node{Van't Hoff};     & \node{Photochemical};\\};
\node[parameter,below right = 1mm,
        align=left]          (par)      at (kineval.south)      {Temperature     (\verb!const StateType &T!)\\
                                                                 photon flux     (\verb!const ParticleFlux<VectorCoeffType> &pf!) \\
